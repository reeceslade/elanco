...
<?php
$api_url = 'https://engineering-task.elancoapps.com/api/resources/';
$applications = json_decode(file_get_contents($api_url), true);

$order = isset($_GET['order']) ? $_GET['order'] : '';

$application_name = isset($_GET['resource-name']) ? $_GET['resource-name'] : '';

if (!empty($_GET['resource-name'])) {
  $resource_name = str_replace(' ', '%20', $_GET['resource-name']);
  $resources_url = $api_url . $resource_name;
  ...
  ?>
	<h2><?php echo $_GET['resource-name'];?></h2>
  <a href="index.php">
  ...
  <th>Consumed Quantity
   <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="GET">
                    <input type="hidden" name="resource-name" value="<?php echo $application_name; ?>">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
		...
		<?php foreach ($resources as $resource) { ?>
			<tr>
				...
				<td><?php echo $resource['MeterCategory']; ?></td>
        <td>  <?php $meter_category_url = 'https://elanco.reeceslade.repl.co/applications.php?application-name=' . urlencode($resource['ResourceGroup']); ?>
      <a href="<?php echo $meter_category_url; ?>"><?php echo $resource['ResourceGroup']; ?></a></td>
				<td><?php echo $resource['ResourceLocation']; ?></td><td>
    <?php 
      $tags = $resource['Tags'];
      $formattedTags = '';
		...
		<tr>
			...
			<td>
				<form action="" method="GET">
					<input type="hidden" name="resource-name" value="<?php echo $application; 
          //loop through application url and put the names into a table then whe we click view resources?>">
